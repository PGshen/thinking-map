...
- **依赖关系仅限于同一父节点下的子节点之间，且不可跨层**

#### 布局原则

1. **层级清晰**：主线为树状/分层结构，便于理解主问题-子问题关系。
2. **依赖关系可见**：依赖边仅出现在同一父节点下的子节点之间，避免跨层/跨分支的混乱。
3. **可扩展性**：节点多时依然可读、可交互。
...
- 主线结构一目了然，用户能直观看到“问题-子问题-子子问题”的分解过程

##### 2. 依赖关系用“有向边”连接（仅限同层同父）
- 依赖边只允许连接同一父节点下的子节点（即同层同父）
- 不允许跨层或跨不同父节点的依赖
- 依赖边样式建议与主线边区分（如虚线、半透明、不同颜色）
- 箭头避免与主线重叠，必要时可做曲线/弯折

##### 3. 层级与依赖兼容的布局算法
- 依然推荐**Sugiyama分层布局算法**（Layered DAG Layout），但在生成依赖边时需校验仅限同父同层
- ReactFlow、Cytoscape.js、Dagre等库均可实现，需在数据处理阶段限制依赖边的生成
...
graph TD
  ...
  D -.-> E
```
- 实线为主树结构，虚线为同父同层依赖关系（如 D 和 E 之间）

---

#### 参考实现
- **ReactFlow**：支持自定义布局，可结合 dagre-layout 实现分层DAG，依赖边生成时需校验同父同层
- **Cytoscape.js**：内置多种DAG布局算法，依赖边生成时需自定义过滤
- **d3-dag**：专注于DAG的分层布局，依赖边生成时需自定义限制
