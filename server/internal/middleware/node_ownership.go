...
func NodeDetailOwnershipMiddleware(nodeDetailRepo repository.NodeDetail, nodeRepo repository.ThinkingNode, mapRepo repository.ThinkingMap) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		detail, err := nodeDetailRepo.FindByID(c.Request.Context(), detailID)
		if err != nil || detail == nil {
			c.JSON(http.StatusNotFound, gin.H{"code": http.StatusNotFound, "message": "node detail not found"})
			c.Abort()
			return
		}
		node, err := nodeRepo.FindByID(c.Request.Context(), detail.NodeID)
		if err != nil || node == nil {
			c.JSON(http.StatusNotFound, gin.H{"code": http.StatusNotFound, "message": "node not found"})
			c.Abort()
			return
		...
		mapObj, err := mapRepo.FindByID(c.Request.Context(), node.MapID)
		if err != nil || mapObj == nil {
			c.JSON(http.StatusNotFound, gin.H{"code": http.StatusNotFound, "message": "map not found"})
			c.Abort()
			return
		}
		if mapObj.UserID != userID {
			c.JSON(http.StatusForbidden, gin.H{"code": http.StatusForbidden, "message": "forbidden: node detail does not belong to user"})
			c.Abort()
			return
		}
